...
///
/// Not all events should be broadcasted through
/// the [EventBus] but only those of
/// general interest.
///
...
///
@Deprecated(
  'Only authInterceptor may use EventBus. Each UI event fired in this bus '
  'has no guarantee that will be shown on correct page at correct time!',
)
class EventBus {
  ...
  /// completed.
  EventBus() : streamController = StreamController.broadcast(sync: false);

  /// Listens for events of Type [T] and its subtypes.
  ///
  /// The method is called like this: myEventBus.on<MyType>();
  ///
  /// If the method is called without a type parameter,
  /// the [Stream] contains every
  ...
  /// and if they pause, only the pausing
  /// listener is affected. A paused listener
  /// will buffer events internally until
